...
class TransactionController(Controller):
    ...
    async def list_transactions(self, client: httpx.AsyncClient) -> list[Transaction]:
        ...
        subtransactions = self.construct_df(budget.subtransactions).select(
            "amount", "payee_id", "category_id", id="transaction_id"
        )
        transactions = (
            self.construct_df(budget.transactions)
            .select("id", "date", "amount", "account_id", "payee_id", "category_id")
            .update(subtransactions, how="left", on="id")
        )

        accounts = self.construct_df(budget.accounts).select(
            account_id="id", account="name"
        )

        payees = self.construct_df(budget.payees).select(payee_id="id", payee="name")

        category_groups = self.construct_df(budget.category_groups).select(
            category_group_id="id", category_group="name"
        )
        categories = (
            self.construct_df(budget.categories)
